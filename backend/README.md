...
适用于在本机或任意目录用 Compose 启动全栈（MySQL、Consul、Portal、Runner、Web）。数据目录默认在生产环境为 `/usr/yunji/cloudiac`；本地可通过环境变量 **`CLOUDIAC_DEPLOY_ROOT`** 指向项目下的目录（例如 `.cloudiac-deploy`），无需使用 `/usr/yunji/cloudiac`。

**首次准备**

```sh
mkdir -p .cloudiac-deploy/var/consul .cloudiac-deploy/var/mysql
cp configs/dotenv.sample .cloudiac-deploy/.env
# 编辑 .cloudiac-deploy/.env：至少填写 IAC_ADMIN_PASSWORD、SECRET_KEY、PORTAL_ADDRESS、CONSUL_ADDRESS 等必填项（说明见文件内注释；容器内互通时 CONSUL_ADDRESS 可设为 consul:8500）。本地体验可将 IAC_ADMIN_PASSWORD 设为下方「Web 默认登录」中的示例密码。
```

**启动**

```sh
export CLOUDIAC_DEPLOY_ROOT="$PWD/.cloudiac-deploy"
docker compose -f docker/docker-compose.yml --env-file .cloudiac-deploy/.env up -d
```

首次启动时 `iac-portal` 可能在 MySQL 就绪前短暂重启，属正常现象。
...
| 组件 | 地址 |
|------|------|
| Web 控制台 | http://localhost（映射容器 80 端口） |
| Portal API | http://localhost:9030 |
| Consul UI | http://localhost:8500 |
| Runner | http://localhost:19030 |

**Web 默认登录**

与 `configs/dotenv.sample` 一致时，管理员邮箱默认为 **admin@example.com**。密码没有内置默认值，由 `.env` 中的 **`IAC_ADMIN_PASSWORD`** 在首次初始化时写入；须自行设置（要求长度大于 8 且包含字母、数字、特殊字符）。本地体验可将该变量设为示例密码 **CloudIac#Dev2026**，则 Web 登录为：

- 邮箱：`admin@example.com`
- 密码：`CloudIac#Dev2026`

生产环境请务必改为强密码。`.cloudiac-deploy/` 已加入 `.gitignore`，请勿将含真实密钥的配置提交到仓库。更完整的部署说明见 [容器部署文档](docs/mkdocs/product-deploy/container.md)。
